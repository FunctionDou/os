...
[GLOBAL glb_mboot_ptr]  ; 向外部声明 struct multiboot * 变量 在 <start> 开始的时候, 执行 mov DWORD PTR ds: 0x10a00c ebx语句时, 将ebx指向的地址存给multiboot_t结构体中, 保存全局变量
...
start:
    ...
    mov [glb_mboot_ptr], ebx ; 将 ebx 中存储的指针存入全局变量, 存放了multiboot_t结构体的指针
